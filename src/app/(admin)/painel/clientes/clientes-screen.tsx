...
export function ClientesScreen() {
  ...
  async function reload() {
    setLoading(true);
    const data = await apiFetch<CustomerListItem[]>("/customers");
    setItems(data);
    setLoading(false);
  }
  ...
  async function toggleActive(item: CustomerListItem) {
    try {
      if (item.active) {
        await apiFetch(`/customers/${item.id}`, { method: "DELETE" });
      } else {
        await apiFetch(`/customers/${item.id}`, {
          method: "PATCH",
          body: JSON.stringify({
            name: item.name,
  ...
  return (
    <div>
      <PageHeader
        title="Clientes"
        description="Cadastro usado no PDV e nas vendas."
        actions={
          <Button
            onClick={() => {
              setForm(EMPTY_FORM);
